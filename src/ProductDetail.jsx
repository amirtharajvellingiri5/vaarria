                ...
                {activeMenu === item.name && (
                  <div className='absolute top-full left-0 mt-2 bg-white shadow-xl rounded-xl py-2 min-w-48 border border-pink-100 z-50'>
                    {item.submenu.map((sub) => (
                      <a key={sub} href='#' className='block px-4 py-2 text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition'>
                        {sub}
                      </a>
                    ))}
                  </div>
      ...
      {isOpen && (
        <div className='md:hidden border-t border-pink-100'>
          <div className='px-4 pt-2 pb-3 space-y-1 bg-pink-50/50'>
            {menuItems.map((item) => (
              <div key={item.name}>
                <a href='#' className='block px-3 py-2 text-gray-700 hover:bg-pink-100 rounded font-medium text-sm'>
                  {item.name}
                </a>
              </div>
            ))}
...
const HARDCODED_PRODUCT = {
  ...
  colors: [{ name: 'Navy Blue', hex: '#1a237e', active: true }, { name: 'Maroon', hex: '#7b1fa2', active: false }, { name: 'Teal', hex: '#00695c', active: false }],
  details: { 'Style Code': 'SNG-KRT-24386', Occasion: 'Ethnic, Casual', 'Fabric Care': 'Gentle Machine Wash / Hand Wash', Neck: 'V-Neck', Sleeve: 'Three-Quarter Sleeves', Pattern: 'Embroidered', 'Length Type': 'Regular', 'Fit Type': 'Straight', Fabric: 'Viscose Rayon', Color: 'Navy Blue', 'Country of Origin': 'India' },
  description: 'Crafted from premium viscose rayon, this Sangria kurta features intricate ethnic motif embroidery with detailed thread work at the yoke and hem.',
  highlights: ['Premium Viscose Rayon fabric', 'Intricate ethnic motif embroidery', 'Thread work detailing at yoke & hem', 'Straight fit silhouette', 'Three-quarter sleeves', 'V-neck with button detail'],
  deliveryInfo: { pincode: '560001', days: '2-3 Business Days', cod: true },
  offers: [{ icon: '💳', title: 'Bank Offer', desc: '10% off on HDFC Bank Credit Cards' }, { icon: '🎁', title: 'Special Price', desc: 'Get extra 5% off (price inclusive of discount)' }, { icon: '🔄', title: 'Easy Returns', desc: '30-day return policy. No questions asked.' }],
  ratings: { overall: 4.3, breakdown: [{ label: '5 ★', pct: 55 }, { label: '4 ★', pct: 22 }, { label: '3 ★', pct: 12 }, { label: '2 ★', pct: 6 }, { label: '1 ★', pct: 5 }] },
  reviews: [{ id: 1, user: 'Priya M.', rating: 5, date: 'Nov 2024', title: 'Absolutely stunning!', body: 'The embroidery work is exquisite and the fabric is super soft. Fits perfectly as a straight kurta. Highly recommend!', helpful: 42 }, { id: 2, user: 'Ananya S.', rating: 4, date: 'Oct 2024', title: 'Good quality, great colour', body: 'The navy blue is rich and deep, exactly as shown. Stitching quality is excellent. Slightly long but wearable.', helpful: 28 }, { id: 3, user: 'Deepa R.', rating: 5, date: 'Oct 2024', title: 'Worth every rupee!', body: "Got compliments from everyone at the function. The thread work looks premium and doesn't feel cheap at all.", helpful: 19 }],
}
...
async function fetchProduct(productId) {
  const productFromApi = {
    ...
    price: 3999.0,
    images: ["IMG-20220416-WA0000.jpg", "IMG-20220416-WA0001.jpg", "IMG-20220416-WA0002.jpg", "IMG-20220416-WA0005.jpg", "IMG-20220416-WA0006.jpg"],
    stock: 14
  }
  await new Promise((r) => setTimeout(r, 500)) // simulate API
  return { ...HARDCODED_PRODUCT, ...productFromApi, name: productFromApi.title, images: productFromApi.images.map((img) => `https://cdn.aarria.com/app/images/${img}`) }
}
...
function ImageSlider({ images, placeholderImages, initialIndex, onClose }) {
  ...
  const getImg = (i) => imgErrors[i] ? placeholderImages[i] || `https://placehold.co/1080x1440/ec4899/ffffff?text=Image+${i + 1}` : images[i]
  ...
  return (
    <>
      <style>{`
        ...
        /* CRITICAL: Replicating the exact layout styles from the main component */
      ...
      <div className="pdp-page">
        ...
        <div className="pdp-breadcrumb">
          ...
          <span className="pdp-breadcrumb-sep" style={{color:'#d1d5db'}}>›</span>
        ...
        <div className="pdp-outer">
          ...
          <div className="pdp-info-col">
            ...
            {/* Rating */}
            <div className="pdp-rating-row" style={{display:'flex', alignItems:'center', gap:10, marginTop:12}}>
            ...
            {/* Price */}
            <div className="pdp-price-row" style={{display:'flex', alignItems:'baseline', gap:14, marginTop:18}}>
            ...
            {[...Array(3)].map((_, i) => (
              <div key={i} className="pdp-offer-row" style={{display:'flex', alignItems:'flex-start', gap:10, padding:8, borderRadius:8, background:'#fffbf5', border:'1px solid #fef3c7', marginBottom:7}}>
...
export default function ProductDetail({ productId = '24386974' }) {
  ...
  return (
    <>
        ...
        <div className='pdp-outer'>
          ...
          <div className='pdp-images-col'>
            {product.images.map((img, i) => (
              <div key={i} className='pdp-img-item' onClick={() => openSlider(i)}>
                <img src={getImg(i)} alt={`${product.name} — view ${i + 1}`} onError={() => setImgErrors((e) => ({ ...e, [i]: true }))} />
          ...
          <div className='pdp-info-col'>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', flexShrink: 0 }}><Share2 size={18} color='#9ca3af' /></button>
            </div>
            ...
            <div className='pdp-sizes'>
              {product.sizes.map((s) => {
                ...
                return (
                  <button key={s} className={`pdp-size-btn ${selectedSize === s ? 'selected' : ''}`} disabled={!avail} onClick={() => avail && setSelectedSize(s)}>{s}</button>
                )
              })}
            </div>
            ...
            {/* CTA */}
            <div className='pdp-cta'>
              <button className={`pdp-btn-bag ${addedToBag ? 'added' : ''}`} onClick={handleAddToBag} disabled={!selectedSize}>
                <ShoppingBag size={18} />
                {addedToBag ? '✓ Added to Bag' : 'Add to Bag'}
              </button>
              <button className={`pdp-btn-wishlist ${wishlist ? 'active' : ''}`} onClick={() => setWishlist((w) => !w)}>
            ...
            <div className='pdp-delivery'>
              ...
              <div className='pdp-pincode-row'>
                <input className='pdp-pincode-input' value={pincode} onChange={(e) => setPincode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder='Enter pincode' maxLength={6} />
                <button className='pdp-pincode-btn'>Check</button>
              </div>
              <div className='pdp-delivery-ok'>✓ Estimated delivery in {product.deliveryInfo.days}{product.deliveryInfo.cod && ' · COD available'}</div>
            </div>
            ...
            <div className='pdp-accordion'>
              {accordionSections.map((sec) => (
                <div key={sec.key} className='pdp-accordion-item'>
                  <button className='pdp-accordion-trigger' onClick={() => setExpandedSection(expandedSection === sec.key ? null : sec.key)}>
                    <span className='pdp-accordion-label'>{sec.label}</span>
                    {expandedSection === sec.key ? <ChevronUp size={16} color='#ec4899' /> : <ChevronDown size={16} color='#9ca3af' />}
                  </button>
                  {expandedSection === sec.key && <div className='pdp-accordion-body'>{sec.content}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Image slider modal */}
      {sliderOpen && (
        <ImageSlider images={product.images} placeholderImages={product.placeholderImages} initialIndex={sliderIndex} onClose={() => setSliderOpen(false)} />
      )}
